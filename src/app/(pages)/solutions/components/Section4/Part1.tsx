...
export default function Part1() {
    const visualContent = (
        <motion.div
            ...
            className="w-full rounded-2xl bg-white border border-[#f3f4f6] p-6 shadow-xl flex flex-col items-start"
        >
            <div className="flex flex-row items-center justify-between w-full pb-4 border-b border-gray-100 mb-4">
                <h4 className="text-lg font-bold text-[#111827]">Agency Dashboard</h4>
            </div>
            ...
            <div className="flex flex-row flex-wrap items-center gap-1 py-2 px-2">
                {part1Column.map((data, index) => (
                    <motion.div
                        ...
                        className="flex-none"
                    >
                        <div className="flex flex-col items-center w-[100px] sm:w-[120px] bg-[#f9fafb] rounded-md px-4 py-2 hover:-translate-y-1 transition-all duration-300 hover:bg-gray-200">
                            <h1 className="text-[#9ca3af] text-xs sm:text-sm text-center">{data.title}</h1>
                            <h2 className="font-bold text-[#111827] text-lg sm:text-xl text-center">{data.no}</h2>
                        </div>
                    </motion.div>
                ))}
            </div>
            ...
            <motion.hr
                ...
                viewport={{ once: true }}
                transition={{ duration: 0.5, delay: 0.15 }}
                className="border border-[#e5e7eb] w-full my-3"
            />
            ...
            >
                <h1 className="text-[#111827] text-sm text-left">Client A</h1>
                <div className="flex flex-row items-center gap-x-12">
                    <h2 className="text-[#16a34a] text-sm text-right font-medium">32.8%</h2>
                    <h3 className="text-[#111827] text-sm text-right font-medium">120</h3>
                </div>
            </motion.div>
        </motion.div>
    )
